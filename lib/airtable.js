...
/**
 * Escape user input for Airtable filter formulas.
...
function mapSparkTeamRecord(record) {
  ...
  return {
    ...
    // Personal details
    address: f['Address'] || '',
    dateOfBirth: f['Date of Birth'] || '',
    mobile: f['Mobile'] || '',
    bankAccount: f['Bank Account'] || '',
    // Financial fields
    ...
    // Onboarding status
    personalDetailsComplete: f['Personal Details Complete'] === true,
    irdComplete: f['IRD Complete'] === true,
    kiwiSaverComplete: f['KiwiSaver Complete'] === true,
  };
}
...
export async function getSparkTeamByTribe(tribe) {
  const safeTribe = escapeFormulaValue(tribe);
  const data = await airtableFetch(TABLES.SPARK_TEAM, {
    filterByFormula: `AND({Tribe} = '${safeTribe}', NOT({Status} = 'Finished'))`,
  });
...
/**
 * Create a new Spark Team member record
 */
export async function createSparkTeamMember(data) {
  const url = `${BASE_URL}/${encodeURIComponent(TABLES.SPARK_TEAM)}`;

  // Map from app field names to Airtable field names
  const fields = {
    'Name': data.name,
    'Email': data.email,
    'Mobile': data.mobile || '',
    'Job title': data.jobTitle || '',
    'Tribe': data.tribe || 'Brand',
    'Chapter Lead': data.chapterLead || '',
    'Start date': data.startDate || null,
    'End date': data.endDate || null,
    'Status': data.status || 'Ongoing',
    'Access': 'Employee',
    'Salary': data.salary || 90000,
    'Hours': data.hours || 40,
    'Kiwisaver': data.kiwiSaver ?? true,
    'Allowances': data.allowances || 0,
    'Margin %': [`${data.marginPercent || 5}%`], // Array for select field
  };
  
  // Remove null/empty date fields (Airtable doesn't like empty strings for dates)
  if (!fields['Start date']) delete fields['Start date'];
  if (!fields['End date']) delete fields['End date'];
  ...
  return mapSparkTeamRecord(record);
}
...
export async function getAdminByEmail(email) {
  ...
  const data = await airtableFetch(TABLES.ADMINS, {
    filterByFormula: `AND({Email} = '${safeEmail}', {Active} = 1)`,
    maxRecords: 1,
  });
  if (!data.records.length) return null;
...
export async function getPayments(fiscalYear = null) {
  const params = fiscalYear
    ? { filterByFormula: `{fiscal_year} = '${escapeFormulaValue(fiscalYear)}'` }
    : {};
...
export async function getValidPasscode(email, code) {
  const safeEmail = escapeFormulaValue(email);
  const safeCode = escapeFormulaValue(code);
  const data = await airtableFetch(TABLES.PASSCODES, {
    filterByFormula: `AND({email}='${safeEmail}', {code}='${safeCode}', NOT({used}))`,
    maxRecords: 1,
  });
...
function parsePercent(value) {
  if (!value) return 5;
  // Multiple Select fields return an array like ["5%"]
  if (Array.isArray(value)) {
    value = value[0] || '5%';
  }
  if (typeof value === 'number') {
    // If it's a decimal like 0.05, convert to whole number (5)
